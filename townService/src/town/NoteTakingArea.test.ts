...
describe('NoteTakingArea', () => {
    ...
    beforeEach(() => {
        ...
        testNote1 = { id: 'note-1', title: 'Test Note 1', content: '<p>This is a test note.</p>' };
        testNote2 = { id: 'note-2', title: 'Test Note 2', content: '<p>This is another test note.</p>' };
    });
    describe('add', () => {
    ...
    describe('remove', () => {
        ...
        it('Clears the notes when the last occupant leaves', () => {
            ...
            // @ts-ignore
            expect(testArea._occupants.length).toBe(0);
            expect(lastEmittedUpdate).toEqual({
                id,
                notes: undefined,
                occupants: [],
    ...
    describe('Update Notes', () => {
        it('Updates the notes in the area and emits an interactableUpdate event', () => {
            testArea.notes = [testNote1, testNote2];
            testArea['_emitAreaChanged']();
            const lastEmittedUpdate = getLastEmittedEvent(townEmitter, 'interactableUpdate');
            expect(lastEmittedUpdate).toEqual({
                id,
